...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <div class="float-end">
                    <a href="{{route('sale')}}" class="btn btn-de-primary btn-sm"><i class="mdi mdi-view-list"></i> All Sales</a>
                </div>
    ...
    <form action="{{ route('sale_store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-3">
                                ...
                                <select id="customer" class="form-select custom_select" name="customer" required>
                                    <option value="">Select Customer</option>
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id }}" {{ old('customer') == $customer->id ? "selected" : "" }}>{{ $customer->name }} (0{{ $customer->phone }}), {{ $customer->code }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-3">
                            ...
                            <div class="col-lg-3">
                                ...
                                <input type="date" name="sale_date" class="form-control" id="" value="{{ old('sale_date') ?? date("Y-m-d") }}" placeholder="">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table-sm mb-0">
                                <thead class="thead-light">
                                    <tr class="border-bottom item_rows">
                                        ...
                                        <th class="p-2" style="width: 7%;">Expiry Date (Show Lot No)</th>
                                ...
                                <tbody>
                                        ...
                                        @foreach (old('items') as $key => $item)
                                            <tr class="border-bottom item_rows" id="item_row_{{ $key }}">
                                                ...
                                                <td class="p-2">
                                                    ...
                                                    <option value="" data-item-image="{{ asset('assets/images/items/item.jpg') }}" data-carton-size="0" data-sale-price="0">Select Item</option>
                                                        @foreach ($items as $item)
                                                            <option value="{{ $item->id }}" {{ old('items')[$key] == $item->id ? "selected" : "" }} data-item-image="{{ asset('assets\images\\' . $item->image) }}" data-carton-size="{{ $item->carton_size }}" data-sale-price="{{ $item->selling_price }}">{{ $item->name }}, {{ $item->code }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td class="p-2">
                                    ...
                                    @else
                                        <tr class="border-bottom item_rows" id="item_row_0">
                                            ...
                                            <td class="p-2">
                                                ...
                                                <option value="" data-item-image="{{ asset('assets/images/items/item.jpg') }}" data-carton-size="0" data-sale-price="0">Select Item</option>
                                                    @foreach ($items as $item)
                                                        <option value="{{ $item->id }}" data-item-image="{{ asset('assets\images\\' . $item->image) }}" data-carton-size="{{ $item->carton_size }}" data-sale-price="{{ $item->selling_price }}">{{ $item->name }}, {{ $item->code }}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <tr id="excess_amount">
                                        <td class="text-end fw-bold">Customer Credit <span class="text-muted"></span></td>
                                        <td colspan="2"></td>
                        ...
                        <div class="row justify-content-center mt-4">
                            <div class="col-lg-3">
                                <input class="form-control btn btn-purple" type="Submit" value="Sale Items">
                            </div>
                        </div>
                    </div>
                </div>
...
@section('script')
    <script>
        ...
        function itemChanged(selectElement) {
            ...
            var salePrice = selectOption.data('sale-price');
